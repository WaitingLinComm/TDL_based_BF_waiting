function [w_frost, outputpower, epsilon, SINR_buffer] = Frost_Beamformer(w_opt, C, g, M ,J ,nu, iterations, N_Omega, delta, Rx, Rs, Rv, Reta, X_frames, S_frames, V_frames, Eta_frames) 
...
w_0     = C * inv(C' * C + delta * eye(N_Omega)) * g; % size:(M * J, 1).  % with diagonal loading. 
% w_0     = C * inv(C' * C ) * g;   % size:(M * J, 1) % without diagonal loading.
W(:,1)  = w_0;

P       = eye(M * J) - C * inv(C' * C + delta * eye(N_Omega)) * C'; % size:(M * J, M * J) 
